(function () {
  const currentPage = window.location.pathname.split("/").pop() || "index.html";

  const ACADEMY_URL = "https://reotravelacademy.vercel.app/";

  const NAV_LINKS = [
    { href: "../index.html", label: "Home" },
    { href: "about.html", label: "About" },
    { href: "services.html", label: "Services" },
    { href: "gallery.html", label: "Gallery" },
    { href: "blog.html", label: "Blog" },
    { href: "contact.html", label: "Contact" },
    { href: ACADEMY_URL, label: "🎓 Academy", external: true },
  ];

  const isRoot = !window.location.pathname.includes("/pages/");
  function resolveHref(href) {
    if (/^https?:\/\//i.test(href)) return href;
    return isRoot ? href.replace("../", "") : href;
  }

  const base = isRoot ? "" : "../";
  const logoPath = `${base}asset/logo.png`;
  const assetPath = (f) => `${base}asset/${f}`;

  function navLinksHTML() {
    return NAV_LINKS.map((l) => {
      const href = resolveHref(l.href);
      const file = href.split("/").pop();
      const act = !l.external && file === currentPage ? " active" : "";
      const isCta = l.label === "Contact";
      const extra = l.external
        ? ` target="_blank" rel="noopener" class="nav-academy-btn${act}"`
        : ` class="${isCta ? "nav-cta" : ""}${act}"`;
      return `<li><a href="${href}"${extra}>${l.label}</a></li>`;
    }).join("");
  }

  function mobileLinksHTML() {
    return NAV_LINKS.map((l) =>
      l.external
        ? `<a href="${resolveHref(l.href)}" target="_blank" rel="noopener" class="mobile-academy-btn">${l.label} ↗</a>`
        : `<a href="${resolveHref(l.href)}">${l.label}</a>`,
    ).join("");
  }

  const navTarget = document.getElementById("navbar-placeholder");
  if (navTarget) {
    navTarget.outerHTML = `
<nav id="navbar">
  <a class="nav-logo" href="${resolveHref("../index.html")}">
    <img src="${logoPath}" alt="REO Travels & Tour Logo"/>
  </a>
  <ul class="nav-links">${navLinksHTML()}</ul>
  <div style="display:flex;align-items:center;gap:12px;">
    <div class="theme-toggle-wrap" title="Toggle light/dark mode">
      <span class="theme-icon" id="theme-icon-sun">☀️</span>
      <button class="theme-toggle" id="theme-toggle-btn" aria-label="Toggle theme"></button>
      <span class="theme-icon" id="theme-icon-moon">🌙</span>
    </div>
    <button class="hamburger" aria-label="Menu">
      <span></span><span></span><span></span>
    </button>
  </div>
</nav>
<div class="mobile-menu">
  <button class="mobile-menu-close" id="mobile-menu-close" aria-label="Close menu">✕</button>
  ${mobileLinksHTML()}
  <button class="mobile-theme-btn" id="mobile-theme-btn">
    <span id="mobile-theme-icon">☀️</span> Switch to Light Mode
  </button>
</div>`;
  }

  const footTarget = document.getElementById("footer-placeholder");
  if (footTarget) {
    footTarget.outerHTML = `
<footer id="footer">
  <div class="footer-top">
    <div class="footer-brand">
      <a class="nav-logo" href="${base}index.html">
        <img src="${logoPath}" alt="REO Travels Logo" style="height:60px;"/>
      </a>
      <p style="margin-top:12px;">Birthing Travel Dreams since 2022. Your premier gateway to extraordinary destinations across 25+ countries.</p>
      <div class="footer-socials">
        <a href="https://www.instagram.com/reo_travels?igsh=cHVzanJmNDMyM3k0&utm_source=qr" class="footer-social-icon" aria-label="Instagram"> <img src="${assetPath("ig.png")}"/></a>
        <a href="https://www.facebook.com/share/185pypd2Cn/?mibextid=wwXIfr" class="footer-social-icon" aria-label="Facebook"><img src="${assetPath("face.png")}"/></a>
        <a href="https://wa.me/2348172926565" target="_blank" class="footer-social-icon" aria-label="WhatsApp"><img src="${assetPath("whatsapp.png")}"/></a>
        <a href="mailto:info@reotravelsandtours.org" class="footer-social-icon" aria-label="Email"><img src="${assetPath("email.png")}"/></a>
      </div>
    </div>
    <div class="footer-col">
      <h4>Explore</h4>
      <ul>
        <li><a href="${base}services.html">Our Services</a></li>
        <li><a href="${base}gallery.html">Gallery</a></li>
        <li><a href="${base}blog.html">Travel Blog</a></li>
        <li><a href="${base}about.html">Our Story</a></li>
        <li><a href="${ACADEMY_URL}" target="_blank" rel="noopener" style="color:var(--gold);font-weight:600;">🎓 REO Travel Academy ↗</a></li>
      </ul>
    </div>
    <div class="footer-col">
      <h4>Services</h4>
      <ul>
        <li><a href="${base}services.html">Visa Guidance</a></li>
        <li><a href="${base}services.html">Flight Bookings</a></li>
        <li><a href="${base}services.html">Hotel Reservations</a></li>
        <li><a href="${base}services.html">Airport Transfers</a></li>
        <li><a href="${base}services.html">Travel Insurance</a></li>
        <li><a href="${base}services.html">Group Packages</a></li>
      </ul>
    </div>
    <div class="footer-col">
      <h4>Contact</h4>
      <ul>
        <li><a href="tel:+2348172926565">+234 817 292 6565</a></li>
        <li><a href="tel:+2348102850897">+234 810 285 0897</a></li>
        <li><a href="mailto:info@reotravelsandtours.org">info@reotravelsandtours.org</a></li>
        <li><a href="${base}contact.html">Book a Consultation</a></li>
      </ul>
    </div>
  </div>
  <div class="footer-bottom">
    <p>&copy; 2025 REO Travels &amp; Tour. All rights reserved.</p>
    <div style="display:flex;gap:18px;flex-wrap:wrap;">
      <a href="${base}privacy.html">Privacy Policy</a>
      <a href="${base}terms.html">Terms &amp; Conditions</a>
    </div>
  </div>
</footer>`;
  }

  const waTarget = document.getElementById("wa-placeholder");
  if (waTarget) {
    waTarget.outerHTML = `<a href="https://wa.me/2348172926565?text=Hello%20REO%20Travels%20%26%20Tour%2C%20I%20want%20to%20make%20an%20enquiry" class="wa-float" target="_blank" aria-label="Chat on WhatsApp"></a>`;
  }

  const preTarget = document.getElementById("preloader-placeholder");
  if (preTarget) {
    preTarget.outerHTML = `
<div id="preloader">
  <div class="pre-logo"><img src="${logoPath}" alt="REO Travels"/></div>
  <div class="pre-track"><div class="pre-fill"></div></div>
  <p class="pre-text">Birthing Travel Dreams&hellip;</p>
</div>`;
  }
})();
